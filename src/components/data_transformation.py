    ...
    def get_data_transformer_object(self):
            try:
                ...
                ordinal_pipeline = Pipeline(
                    steps=[('ordinal',OrdinalEncoder(categories=[['Mild','Strong']]))]
                )

                # gender,city --->onehot encoder
                onehot_pipeline = Pipeline(
                    steps=[("onehot",OneHotEncoder(handle_unknown='ignore',sparse_output=False))]
                    # one hot encoder add a new column and that column consist of zeros and non zeros and that the more zeros then it focus on zeros for preventing this we use sparse_output
                )


                # Now we make a container so that can the numerical pipeline hits on numric data and categorical data cat_data.
